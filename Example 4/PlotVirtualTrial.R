...
PlotVirtualTrial <- function( lVirtualTrial, nTrialID = NA )
{
    ...
    gPlot <- gPlot + theme_minimal()  + theme(legend.position = "top", plot.title=element_text( hjust=0.5, size=16))
    ...
    gPlot <- gPlot + annotate("text", x = ( 200 + lVirtualTrial$nMinQtyOfPats )/2, y=-0.02, label = "Stop Select S", colour="Red")
    gPlot <- gPlot + annotate("text", x=( 200 + lVirtualTrial$nMinQtyOfPats )/2, y=1.02, label = "Stop Select E", colour="Red")
